...
function createRoom(matchmade,mode){const m=normalizeMode(mode);const x={id:rid(),mode:m,players:[],g:newGame(m),ai:false,aiColor:null,difficulty:null,matchmade:!!matchmade,rps:{phase:'rps',choices:{},result:null,winnerPid:null},undoStack:[],proposal:null,rematchInvite:null,chat:[]};rooms.set(x.id,x);return x;}
...
wss.on('connection',ws=>{
  ...
  ws.on('message',raw=>{
    ...
    if(m.action==='create'){
      ...
      const mode=normalizeMode(m.mode),difficulty=['easy','normal','hard'].includes(m.difficulty)?m.difficulty:'normal';x={id:rid(),mode,g:newGame(mode),players:[],ai:true,aiColor:mode==='xiangqi'?'black':'white',difficulty,matchmade:false,rps:{phase:'done',choices:{},result:null,winnerPid:null},undoStack:[],proposal:null,rematchInvite:null,chat:[]};rooms.set(x.id,x);p={ws,pid:pid(),profileId:String(m.profileId||''),name:String(m.name||'玩家1').slice(0,12),avatar:normalizeAvatar(m.avatar),color:mode==='xiangqi'?'red':'black',connected:true};x.players.push(p);x.g.turn=p.color;setTurnDeadline(x);send(p,{type:'room',roomId:null,pid:p.pid,color:p.color,mode:x.mode,difficulty,matchmade:false});send(p,snapshot(x,p));
    }else if(!x||!p)return;
